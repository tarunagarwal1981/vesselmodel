...
# Model selection - matching test code options
...
def get_hull_data(engine, vessel_type):
    ...
    try:
        ...
        # If count is reasonable, proceed with full query - FIXED COLUMN NAMES
...
def separate_data(df):
    ...
    # FIXED: Match test code logic exactly
    ...
    return ballast_df.dropna(), laden_df.dropna()

# FIXED: Match test code train_model function exactly
def train_model(X, y, model_type):
    if len(X) < 5:  # Add minimum data validation like test code
...
# FIXED: Match test code predict function exactly
def predict_performance(model, scaler, poly, input_data, model_type):
    ...
    if model_type == "Linear Regression with Polynomial Features":
        ...
        input_poly = poly.transform(input_scaled)  # transform, not fit_transform!
...
if st.sidebar.button("Generate Predictions"):
    try:
        ...
        # FIXED: Train models with consistent return values
        st.write("Training models...")
        ballast_power_model, ballast_power_scaler, ballast_power_poly = train_model(ballast_df[input_columns], ballast_df['me_power_kw'], selected_model)
        ballast_consumption_model, ballast_consumption_scaler, ballast_consumption_poly = train_model(ballast_df[input_columns], ballast_df['me_consumption_mt'], selected_model)
        laden_power_model, laden_power_scaler, laden_power_poly = train_model(laden_df[input_columns], laden_df['me_power_kw'], selected_model)
        laden_consumption_model, laden_consumption_scaler, laden_consumption_poly = train_model(laden_df[input_columns], laden_df['me_consumption_mt'], selected_model)
        
        # Check if any model training failed
        if any(model is None for model in [ballast_power_model, ballast_consumption_model, laden_power_model, laden_consumption_model]):
            st.error("Some models failed to train due to insufficient data")
            st.stop()
        
        # FIXED: Match test code speed ranges exactly
        ...
        for speed in speed_range:
            ...
            # FIXED: Pass polynomial transformer to prediction function
            ballast_power = predict_performance(ballast_power_model, ballast_power_scaler, ballast_power_poly, input_data, selected_model)[0]
            ballast_consumption = predict_performance(ballast_consumption_model, ballast_consumption_scaler, ballast_consumption_poly, input_data, selected_model)[0]
            laden_power = predict_performance(laden_power_model, laden_power_scaler, laden_power_poly, input_data, selected_model)[0]
            laden_consumption = predict_performance(laden_consumption_model, laden_consumption_scaler, laden_consumption_poly, input_data, selected_model)[0]
